...
- **New `callers` table**: one row per Caller — author ID (PK), display name, avatar URL, last-seen timestamp. Upserted on every ingested callout from envelope data; missing rows for allowlisted author IDs are seeded at trader boot via Discord REST (`GET /users/{id}`, bot token). Boot-time (insert-only, idempotent) rather than a one-time script because production runs in a container with no shell to run scripts against the live database. A manual script wrapping the same function remains for local use.
- **Legacy callout backfill**: `callouts` rows written before author capture carry a null author ID; at boot the trader re-reads the same REST history catch-up uses and fills them in by message ID. Rows history no longer covers stay null and age out of the feed window. There is deliberately no foreign key from `callouts.author_id` to `callers` — the caller upsert is allowed to fail without blocking the trade path, so a hard constraint could reject a valid callout.
